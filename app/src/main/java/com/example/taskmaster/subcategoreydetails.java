...
public class subcategoreydetails extends AppCompatActivity {
    Toolbar toolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        public void onClick(View view) {
            ...
            if(loc.equals("")||tim.equals(""))
                Toast.makeText(subcategoreydetails.this, "Please fill all the fields", Toast.LENGTH_SHORT).show();
            else{
                SharedPreferences sharedPreferences= getSharedPreferences("MyPrefs", Context.MODE_PRIVATE);
                String targetEmail = sharedPreferences.getString("email","email@hotmail.com");
                ordermod order = new ordermod(0,2, targetEmail, loc, 0,tim , "Fahad", "Accepted", "055567678");
                Boolean insert = DB.addOrder(order);
                if(insert){
                    Toast.makeText(subcategoreydetails.this, "Successful Order", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(getApplicationContext(),ActiveOrder.class);
                    startActivity(intent);
                }else{
                    Toast.makeText(subcategoreydetails.this, "Failed to complete the order", Toast.LENGTH_SHORT).show();
                }

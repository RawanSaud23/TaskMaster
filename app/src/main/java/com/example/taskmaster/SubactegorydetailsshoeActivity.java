    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        public void onClick(View view) {
            ...
            if(loc.equals("")||tim.equals(""))
                Toast.makeText(SubactegorydetailsshoeActivity.this, "Please fill all the fields", Toast.LENGTH_SHORT).show();
            else{
                SharedPreferences sharedPreferences= getSharedPreferences("MyPrefs", Context.MODE_PRIVATE);
                String targetEmail = sharedPreferences.getString("email","email@hotmail.com");
                ordermod order = new ordermod(0,1, targetEmail, loc, 0,tim , "Anwar", "Accepted", "05554233");
                Boolean insert = DB.addOrder(order);
                if(insert){
                    Toast.makeText(SubactegorydetailsshoeActivity.this, "Successful Order", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(getApplicationContext(),ActiveOrder.class);
                    startActivity(intent);
                }else{
                    Toast.makeText(SubactegorydetailsshoeActivity.this, "Failed to complete the order", Toast.LENGTH_SHORT).show();
                }
